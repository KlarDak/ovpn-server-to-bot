...
import { pathDirs, vpnManagementPaths } from "../utils/envUtil.js";
import SQLiteClient from "../utils/sqliteUtil.js";
import type { IConnectedUser } from "../interfaces/IConnectedUser.js";
...
export async function getConnectedClients() : Promise<IConnectedUser[]> {
  try {
    ...
    const activeUsersList = await dbFile.get("SELECT * FROM users WHERE status = 'active'");
